...
import random
import math
from uibrick import Block,Ball
...
# 화살표
trgimg = pygame.image.load("picture/triangle.png")
trg = trgimg.get_rect()
trg.center = (screen.get_rect().centerx, 550)

# 막대
stick_image = pygame.image.load('picture/stick.png')
stick_image = pygame.transform.scale(stick_image,(1200,60))
stick = Block(stick_image)
stick.mask = pygame.mask.from_surface(stick.image)


# 배경
BG_LEFT_IMAGE = pygame.image.load("picture/bgleft.png")
BG_RIGHT_IMAGE = pygame.image.load("picture/bgright.png")
...
while done:
    ...
    for ball in ball_list:
        if ball.rect.bottom >= 768:  # 스틱으로 공을 받지 못해 공이 밑으로 나갔을 경우
            ball.rect.centery -= 63  # 공이 벽 안으로 들어가는 오류 방지
            ball.vely *= -1
    ...
    for ball in ball_list:
        if pygame.sprite.collide_mask(ball, stick):
            ...
            ball.velx = math.cos(angle) * 25
            ball.vely = math.sin(angle) * 25
